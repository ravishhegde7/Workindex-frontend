...
function handleExpertSearch(value) {
  ...
  // Show suggestions immediately from local data
  showSearchSuggestions(value);

  // Debounce actual search by 500ms
  searchTimeout = setTimeout(() => {
    const filters = { location: value };
    const activeService = document.querySelector('.filter-chip.active')?.dataset.service;
    if (activeService && activeService !== 'all') filters.service = activeService;
    loadExperts(filters);
  }, 500);
}
...
function showSearchSuggestions(value) {
  ...
  // Match cities from loaded experts
  const cities = [...new Set(
    state.experts
      .map(e => e.location?.city || e.profile?.city)
      .filter(Boolean)
  )];
  cities.forEach(city => {
  ...
  // Match pincodes from loaded experts
  const pincodes = [...new Set(
    state.experts
      .map(e => e.location?.pincode || e.profile?.pincode)
      .filter(Boolean)
  )];
  ...
  const typeLabels = { service: 'Service', city: 'City', pincode: 'Pincode' };
  ...
  suggestionsEl.innerHTML = suggestions.slice(0, 6).map(s => `
    <div onclick="selectSearchSuggestion('${s.value}', '${s.type}')"
      ...
      <div>
        <div style="font-size: 14px; font-weight: 600; color: var(--text);">${s.label}</div>
        <div style="font-size: 11px; color: var(--text-muted);">${typeLabels[s.type]}</div>
      </div>
    </div>
  `).join('');
...
function selectSearchSuggestion(value, type) {
  ...
  if (type === 'service') {
    // Activate the service filter chip
    document.querySelectorAll('.filter-chip').forEach(c => c.classList.remove('active'));
